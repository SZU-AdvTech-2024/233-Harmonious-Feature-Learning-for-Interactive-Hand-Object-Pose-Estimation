...
class HONet(nn.Module):
    def __init__(self, roi_res=32, joint_nb=21, stacks=1, channels=256, blocks=1,
        ...
        #新加部分
        # self.cbam_h = CBAM(256)#空间通道注意力模块,对手增强
        # self.cbam_o = CBAM(256)#空间通道注意力模块,对物体增强
    ...
    def net_forward(self, imgs, bbox_hand, bbox_obj,mano_params=None, roots3d=None):
        ...
        # P2_h=self.cbam_h(P2_h)
        # P2_o=self.cbam_o(P2_o)
        ...
        x_obj = ops.roi_align(P2_o, roi_boxes_inter, output_size=(self.out_res, self.out_res), spatial_scale=1.0 / 4.0,
                        sampling_ratio=-1)
        
        #原版是物体特征，然后用手的框来提取ROIAlign，现在改为物体-手物体交互框。
        # ops.roi_align(P2_o, roi_boxes_hand, output_size=(self.out_res, self.out_res), spatial_scale=1.0/4.0,
        #                  sampling_ratio=-1)  # hand
...
if __name__ == '__main__':
    honet = HONet(roi_res=32, joint_nb=21, stacks=1, channels=256, blocks=1,
                transformer_depth=1, transformer_head=8,
                mano_layer=None, mano_neurons=[1024, 512], coord_change_mat=None,
                reg_object=True, pretrained=True)

    model = HOModel(honet, mano_lambda_verts3d=1.0, mano_lambda_joints3d=1.0,
                    mano_lambda_manopose=1.0, mano_lambda_manoshape=1.0,
                    mano_lambda_regulshape=1.0, mano_lambda_regulpose=1.0,
                    lambda_joints2d=1.0, lambda_objects=1.0)

    with open('model_structure.txt', 'w') as f:
        for name, module in model.named_modules():
            f.write(f"Name: {name}, Module: {module}\n")

    print("模型结构已成功写入 model_structure.txt 文件")